...
def _apply_prompt(batch_dir):
    return "\n".join([
        ...
        "the wiki, mark the batch reviewed, refresh the diagram index, log,",
        "and RECLAIM THE BATCH'S SCRATCH SPACE (step 12: delete video.* and",
        "frames/ and picks/ dirs, keep every json) — that step is mandatory,",
        "a batch left at 10-25 GB is an unfinished run. TC-IL is not a git",
        "repository: there is no commit or push. If picks.json is an empty",
        "map {}, follow the command's empty-selection path: mark the batch",
        "reviewed, reclaim scratch, no wiki changes.",
        "",
        ...
        "  mid-task.",
        "- Stay inside the TC-IL repository, plus the batch dir under",
        "  ~/vk-work for step 12. Run no git operations.",
        "- No emojis anywhere.",
        "",
        "End with the command's delivery-state report: videos done, frames",
        "shipped, Sage catches, space reclaimed before -> after (or 'batch",
        "cleared' on the empty path).",
    ])
