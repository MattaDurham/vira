...
def _visual(value):
    """A source may attach one local visual to its card. Keep the contract
    intentionally small and same-origin: generated art belongs in a Vira-
    served path, never an arbitrary remote embed."""
    if not isinstance(value, dict):
        return None
    kind = str(value.get("kind") or "image")
    src = str(value.get("src") or "")
    if kind not in ("image", "video") or not src.startswith("/") \
            or src.startswith("//"):
        return None
    return {"kind": kind, "src": src,
            "alt": _clip(value.get("alt") or "", WHY_MAX)}


def item(source, raw_id, title, why="", stamp="", actions=(), ref="",
         note="", open="", visual=None):
    ...
    return {
        ...
        "open": str(open or ""),
        # Optional image/video generated by the source's own process. This
        # is presentation context only; it never changes the source actor.
        "visual": _visual(visual),
    }


def context(item_id):
    """Full deciding context for one exact queue item, read on demand.

    The queue stays scan-fast; opening a card reaches back to its owning
    source instead of dumping every source document into `/api/review`.
    This is read-only and never substitutes for the source actor.
    """
    source_key, sep, raw_id = str(item_id or "").partition(":")
    source = SOURCES.get(source_key)
    if not sep or not source:
        raise KeyError(f"no review item {item_id}")
    row = next((r for r in source.read() if r["id"] == item_id), None)
    if not row:
        raise KeyError(f"no review item {item_id}")
    text = "\n\n".join(x for x in (
        row.get("title"), row.get("why"), row.get("note")) if x)
    ref = row.get("ref") or row.get("source_label") or source_key

    if source_key == "inbox":
        root = inbox_notes_dir().resolve()
        path = (root / raw_id).resolve()
        if path.parent != root or not path.is_file():
            raise KeyError(f"no review item {item_id}")
        text = path.read_text(encoding="utf-8", errors="replace")
        ref = str(path)
    elif source_key == "lessons":
        raw = next((x for x in _lessons_rows() if x["_key"] == raw_id), None)
        if raw:
            text = "\n\n".join(str(x) for x in (
                raw.get("text"), raw.get("why") or raw.get("reason"),
                raw.get("note")) if x)
            ref = str(lessons_proposed())
    elif source_key == "ideas":
        from . import ideas
        idea = next((x for x in ideas.list_items() if x["id"] == raw_id), None)
        if idea:
            text = "\n\n".join(x for x in (
                idea.get("text"), idea.get("note")) if x)
    elif source_key == "journal":
        entry, pending = _journal_find(raw_id)
        if entry:
            pieces = [entry.get("text") or ""]
            result = entry.get("result") or {}
            if result.get("summary"):
                pieces.append(result["summary"])
            pieces.extend(str(x) for x in (result.get("actions") or []))
            if pending and pending.get("instruction"):
                pieces.append("Pending instruction:\n" + pending["instruction"])
            text = "\n\n".join(x for x in pieces if x)
    elif source_key == "flags":
        for getter, needle, _label in FLAG_SECTIONS:
            section = _section_text(_read_text(getter()), needle)
            if row.get("title") in section:
                text = section
                ref = f"{getter()}: {needle}"
                break
    elif source_key == "senders":
        candidate = next((c for c in _sender_candidates()
                          if str(c.get("person_id") or c.get("handle")) == raw_id),
                         None)
        if candidate:
            fields = ("name", "handle", "relationship", "evidence",
                      "referral_hint", "msgs", "message_count")
            text = "\n".join(f"{k.replace('_', ' ').title()}: {candidate[k]}"
                              for k in fields if candidate.get(k) not in (None, ""))
    elif source_key == "picker":
        from . import subs_visuals
        state = subs_visuals.status()
        pending = state.get("pending") or {}
        lines = [f"Built: {pending.get('built') or 'unknown'}",
                 f"Batch: {pending.get('batch_dir') or raw_id}"]
        for video in pending.get("videos") or []:
            label = video.get("title") or video.get("slug") or "video"
            channel = video.get("channel") or ""
            lines.append(f"- {label}" + (f" — {channel}" if channel else ""))
        text = "\n".join(lines)
        ref = pending.get("batch_dir") or ref
    return {"id": item_id, "title": row.get("title") or source.label,
            "source": source.label, "ref": ref, "text": text,
            "open": row.get("open") or "", "visual": row.get("visual")}
...
def _section_text(text, heading_needle):
    """The complete matching heading and its body, through the next heading.
    Context views use this while queue rows continue to use compact bullets."""
    lines = (text or "").splitlines()
    out, inside = [], False
    for line in lines:
        m = _HEADING_RE.match(line)
...
SENDERS_TOP = 5
# triage.candidates() probes chat.db per candidate, so repeated Decide-lane
# opens are cached briefly in-process.
SENDERS_CACHE_S = 300
...
def _senders_read():
    ...
    for c in _sender_candidates():
        ...
        rows.append(item(
            ...
            ref="People > Triage",
            open="#triage/" + urllib.parse.quote(
                str(c.get("person_id") or handle), safe="")))
